...
import { ActivityConnectionMapping, FabricTarget, CustomActivityMapping, PipelineConnectionMappings, PipelineMappingSummary, WorkspaceLakehouse } from '../../types';
...
import { WorkspaceItemsService } from '../../services/workspaceItemsService';
import { 
...
export function MappingPage() {
  ...
  const [workspaceLakehouses, setWorkspaceLakehouses] = useState<WorkspaceLakehouse[]>([]);
  ...
  useEffect(() => {
    const loadConnections = async () => {
      if (!state.auth?.accessToken || !state.selectedWorkspace?.id) {
        ...
        setWorkspaceLakehouses([]);
        return;
      }

      setLoadingConnections(true);
      try {
        const [connections, lakehouses] = await Promise.all([
          ExistingConnectionsService.getExistingConnections(
            state.auth.accessToken,
            state.selectedWorkspace.id
          ),
          WorkspaceItemsService.getWorkspaceLakehouses(
            state.auth.accessToken,
            state.selectedWorkspace.id
          )
        ]);
        setExistingConnections(connections);
        setWorkspaceLakehouses(lakehouses);
      } catch (error) {
        console.error('Failed to load existing Fabric connections and workspace lakehouses', error);
        setExistingConnections([]);
        setWorkspaceLakehouses([]);
      } finally {
  ...
  const handleReferenceMapping = (
    ...
    // Update the new referenceId-based mappings (UI + deployment state)
    setPipelineReferenceMappings(prev => {
      ...
      dispatch({
        type: 'SET_PIPELINE_REFERENCE_MAPPINGS',
        payload: nextMappings
      });

      return nextMappings;
    });

    // Keep old mapping format in sync for deployment code paths that still rely on legacy keys.
    const pipelineSummary = pipelineSummaries.find(summary => summary.pipelineName === pipelineName);
    let resolvedLegacyActivityName = referenceId;
    let resolvedLegacyActivityType = 'unknown';
    let resolvedLegacyLinkedService = 'Unknown';
    let resolvedLegacyIndex = 0;

    if (pipelineSummary) {
      for (const group of pipelineSummary.activityGroups) {
        for (const activity of group.activities) {
          const referenceIndex = activity.references.findIndex(ref => ref.referenceId === referenceId);
          if (referenceIndex >= 0) {
            const matchedRef = activity.references[referenceIndex];
            resolvedLegacyActivityName = activity.activityName;
            resolvedLegacyActivityType = activity.activityType;
            resolvedLegacyLinkedService = matchedRef.linkedServiceName;
            resolvedLegacyIndex = referenceIndex;
            break;
          }
        }
      }
    ...
    const legacyUniqueId = buildLegacyUniqueId(
      resolvedLegacyActivityName,
      resolvedLegacyLinkedService,
      resolvedLegacyIndex
    );

    const legacyMapping: ActivityConnectionMapping = {
      activityName: resolvedLegacyActivityName,
      activityType: resolvedLegacyActivityType,
      linkedServiceReference: { name: resolvedLegacyLinkedService },
      selectedConnectionId: connectionId
    };

    setPipelineConnectionMappings(prev => {
      const pipelineMappings = prev[pipelineName] || {};
      const nextLegacyMappings = {
        ...prev,
        [pipelineName]: {
          ...pipelineMappings,
          [legacyUniqueId]: legacyMapping
        }
      };

      dispatch({
        type: 'SET_PIPELINE_CONNECTION_MAPPINGS',
        payload: nextLegacyMappings
      });

      return nextLegacyMappings;
    });
  };
                  ...
                  <CardContent>
                    {generatingSummaries ? (
                      ...
                      <ComponentMappingTableV2
                        ...
                        existingLakehouses={workspaceLakehouses}
