...
import type { ADFComponent, FabricTarget, PipelineMappingSummary, ActivityGroup, ActivityWithReferences, WorkspaceLakehouse } from '../../../types';
...
interface ComponentMappingTableV2Props {
  ...
  existingLakehouses?: WorkspaceLakehouse[];
              ...
              {isPipelineMode ? (
                ...
                visibleItems.map((pipeline: any) => (
                  <PipelineRow
                    ...
                    existingLakehouses={existingLakehouses}
...
function ActivityItem({
  ...
  const [localSourceTypes, setLocalSourceTypes] = useState<Record<string, 'connection' | 'lakehouse'>>({});

  const getDerivedSourceType = (value: string | undefined): 'connection' | 'lakehouse' => {
    return value?.startsWith('lakehouse:') ? 'lakehouse' : 'connection';
  };
    ...
    <div className="px-16 py-3 border-t border-border/50 space-y-2">
      ...
      <div className="space-y-2 ml-4">
        {activity.references.map(ref => {
          ...
          const sourceType = localSourceTypes[ref.referenceId] || getDerivedSourceType(selectedConnectionId);
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="w-64 shrink-0">
                  <RadioGroup
                    value={sourceType}
                    onValueChange={(value) => {
                      const selectedSource = value as 'connection' | 'lakehouse';
                      setLocalSourceTypes(prev => ({
                        ...prev,
                        [ref.referenceId]: selectedSource
                      }));

                      const nextValue = '';
                      setLocalMappings(prev => ({
                        ...prev,
                        [ref.referenceId]: nextValue
                      }));
                      debouncedCallback(pipelineName, ref.referenceId, nextValue);
                    }}
                    className="grid grid-cols-2 gap-2 mb-2"
                  >
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="connection" id={`${ref.referenceId}-connection`} />
                      <Label htmlFor={`${ref.referenceId}-connection`} className="text-xs">Connection</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="lakehouse" id={`${ref.referenceId}-lakehouse`} />
                      <Label htmlFor={`${ref.referenceId}-lakehouse`} className="text-xs">Lakehouse</Label>
                    </div>
                  </RadioGroup>
                  <Select
                  ...
                  >
                    <SelectTrigger className="h-9">
                      <SelectValue placeholder={sourceType === 'lakehouse' ? 'Select lakehouse...' : 'Select connection...'} />
                    </SelectTrigger>
                    <SelectContent>
                      {sourceType === 'lakehouse'
                        ? existingLakehouses.map((lakehouse) => (
                            <SelectItem key={lakehouse.id} value={`lakehouse:${lakehouse.id}`}>
