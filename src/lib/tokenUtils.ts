...
export function validateTokenScopes(token: string): TokenScopes {
  const defaultResult: TokenScopes = {
    ...
    dataPipelineReadWrite: false,
    gatewayReadWrite: false,
    itemReadWrite: false,
    workspaceReadWrite: false,
  ...
  try {
    ...
    const dataPipelineReadWrite = scopes.includes('DataPipeline.ReadWrite.All');
    const gatewayReadWrite = scopes.includes('Gateway.ReadWrite.All');
    const itemReadWrite = scopes.includes('Item.ReadWrite.All');
    const workspaceReadWrite = scopes.includes('Workspace.ReadWrite.All');
    const hasAllRequiredScopes = connectionReadWrite && dataPipelineReadWrite && gatewayReadWrite && itemReadWrite && workspaceReadWrite;

    return {
      connectionReadWrite,
      dataPipelineReadWrite,
      gatewayReadWrite,
      itemReadWrite,
      workspaceReadWrite,
...
export function getScopeDisplayNames(scopes: string[]): { scope: string; displayName: string; hasPermission: boolean }[] {
  ...
  const scopeMap = {
    ...
    'DataPipeline.ReadWrite.All': 'Create and manage data pipelines',
...
/**
 * Returns token expiration date from JWT exp claim.
 */
...
/**
 * Check whether token audience is one of Fabric/Power BI API audiences.
 */
export function isFabricAudienceToken(token: string): boolean {
  const decoded = decodeToken(token);
  ...
  return acceptedAudiences.has(audience);
}

/**
 * Detects if a token has broad delegated/app access for Fabric API.
 */
export function hasFabricBroadAccess(token: string): boolean {
  const decoded = decodeToken(token);
  if (!decoded) {
    return false;
  }

  const scopes = (decoded.scp || '').split(' ').filter(Boolean);
  const hasUserImpersonation = scopes.includes('user_impersonation');
  const hasRoles = Array.isArray(decoded.roles) && decoded.roles.length > 0;

  return hasUserImpersonation || hasRoles;
}
