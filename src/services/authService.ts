...
import { AuthState, ServicePrincipalAuth, isValidTokenResponse, isAzureADError, TenantConfig, InteractiveLoginConfig, ManualTokenLoginConfig } from '../types';
import { fabricScopes } from '../lib/msalConfig';
import { 
  validateAndConfigureTenant, 
  validateTenantIdInput,
...
import { validateAuthenticationResult, validateTokenScopes, decodeToken, isTokenExpired, getTokenExpiration, isFabricAudienceToken, hasFabricBroadAccess } from '../lib/tokenUtils';
...
class AuthService {
  ...
  async loginWithMicrosoft(loginConfig: InteractiveLoginConfig): Promise<AuthState> {
    try {
      ...
      const authState: AuthState = {
        ...
        tokenExpiresAt: response.expiresOn?.toISOString(),
  ...
  async loginWithManualToken(loginConfig: ManualTokenLoginConfig): Promise<AuthState> {
    try {
      const tenantValidation = validateTenantIdInput(loginConfig.tenantId);
      if (!tenantValidation.isValid) {
        throw new Error(tenantValidation.error || 'Invalid tenant ID');
      }

      const rawToken = sanitizeString(loginConfig.accessToken);
      if (!rawToken) {
      ...
      if (!isFabricAudienceToken(accessToken)) {
        throw new Error('Token audience is not valid for Fabric API. Request a token for https://api.fabric.microsoft.com/.default');
      }

      if (isTokenExpired(accessToken)) {
        throw new Error('Provided access token is expired. Generate a new token and try again.');
      }

      const expectedTenant = loginConfig.tenantId.trim().toLowerCase();
      const tokenTenant = String(decodedToken.tenant_id || '').toLowerCase();
      if (tokenTenant && tokenTenant !== expectedTenant) {
        throw new Error('Token tenant does not match the provided tenant ID.');
      }

      const tokenScopes = validateTokenScopes(accessToken);
      const hasBroadAccess = hasFabricBroadAccess(accessToken);
      if (!tokenScopes.hasAllRequiredScopes && !hasBroadAccess) {
        throw new Error('Token does not contain deploy permissions. Ensure it includes Fabric delegated scopes or user_impersonation for Fabric audience.');
      }

      const hasContributorAccess = await this.validateWorkspaceAccess(accessToken, 'default-workspace');
      const tokenExpiration = getTokenExpiration(accessToken);

      const authState: AuthState = {
        isAuthenticated: true,
        accessToken,
        authMode: 'manualToken',
        user: {
          id: String(decodedToken.oid || decodedToken.sub || 'manual-token-user'),
          name: String(decodedToken.name || decodedToken.email || decodedToken.upn || 'Manual Token User'),
          email: String(decodedToken.email || decodedToken.upn || 'manual-token-user@unknown.local'),
          tenantId: loginConfig.tenantId.trim()
        },
        workspaceId: 'default-workspace',
        hasContributorAccess,
      ...
      const errorMessage = extractErrorMessage(error);
      throw new Error(`Manual token authentication failed: ${errorMessage}`);
    }
  }
  ...
  async refreshToken(currentToken: string): Promise<string> {
    try {
      ...
      if (storedAuth?.authMode === 'manualToken') {
        if (isTokenExpired(currentToken)) {
          throw new Error('Manual token expired - please provide a new token.');
        }
        return currentToken;
      }
