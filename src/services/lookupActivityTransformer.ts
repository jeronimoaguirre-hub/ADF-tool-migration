...
/**
 * Service for transforming ADF Lookup activities to Fabric format
 * Converts typeProperties.dataset to datasetSettings with externalReferences
 */
export class LookupActivityTransformer {
  /**
   * Transforms an ADF Lookup activity to Fabric format
   * @param activity The ADF Lookup activity
   ...
   * @returns The transformed Fabric Lookup activity
   */
  transformLookupActivity(
    activity: any,
    pipelineConnectionMappings?: any,
    pipelineReferenceMappings?: Record<string, Record<string, string>>,
    pipelineName?: string
  ): any {
    if (!activity || activity.type !== 'Lookup') {
      return activity;
    }

    console.log(`Transforming Lookup activity: ${activity.name}`);

    // Extract dataset reference from typeProperties.dataset
    const datasetRef = activity.typeProperties?.dataset?.referenceName;
    if (!datasetRef) {
      console.warn(`Lookup activity '${activity.name}' has no dataset reference - returning unchanged`);
      return activity;
    }
    ...
    if (!dataset) {
      console.error(`Dataset '${datasetRef}' not found for Lookup activity '${activity.name}'`);
      return activity;
    }
    ...
    console.log(`Lookup activity '${activity.name}' dataset info:`, {
    ...
    console.log(`Looking up connection for Lookup activity '${activity.name}':`, {
      linkedServiceName,
      pipelineName,
    ...
    // Determine source type (Fabric requires this)
    ...
    const transformedTypeProperties: any = {
      source: {
        ...activity.typeProperties.source,
        type: sourceType  // Add Fabric-required source.type
      },
      datasetSettings,  // Add Fabric datasetSettings
      firstRowOnly: activity.typeProperties.firstRowOnly ?? true
    };

    // Remove ADF dataset reference
    delete transformedTypeProperties.dataset;

    console.log(`✅ Lookup activity '${activity.name}' transformed successfully`, {
   ...
   * Reuses logic from copyActivityTransformer
   */
   ...
   * Priority 2: OLD pipelineConnectionMappings (activity-name-based)
   * Priority 3: Direct linkedService mapping
   * Priority 4: undefined
   */
  private getConnectionIdForLinkedService(
    ...
    if (pipelineReferenceMappings && pipelineName && activityName) {
      ...
      if (pipelineMappings) {
        // Build referenceId: pipelineName_activityName_dataset
   ...
   * Comprehensive mapping for all supported source types
   */
  private convertDatasetTypeToSourceType(datasetType: string): string {
    if (!datasetType) {
    ...
    const specialMappings: Record<string, string> = {
      ...
      'DataWarehouseTable': 'DataWarehouseSource', // Fabric Warehouse
      'MySql': 'MySqlSource',
      'PostgreSql': 'PostgreSqlSource',
      'PostgreSqlV2': 'PostgreSqlV2Source',
      'Oracle': 'OracleSource',
      'DB2': 'Db2Source',
      ...
      'Impala': 'ImpalaSource',
      'Netezza': 'NetezzaSource',
      'Greenplum': 'GreenplumSource',
      ...
      'AzureDataLakeStorageGen2': 'DelimitedTextSource',
      ...
      // NoSQL & Analytics
      'CosmosDb': 'DocumentDbCollectionSource', // Cosmos DB SQL API
      'MongoDb': 'MongoDbSource',
      'Cassandra': 'CassandraSource',
      'AzureTable': 'AzureTableSource',
      'Kusto': 'AzureDataExplorerSource',

      // Web & SaaS
      'HttpServer': 'HttpSource', // For HTTP file downloads
      'RestService': 'RestSource',
      'OData': 'ODataSource',
      'Office365': 'Office365Source',
      'SharePointOnlineList': 'SharePointOnlineListSource',
      'DynamicsCRM': 'DynamicsSource',
      'Salesforce': 'SalesforceSource',
      'SalesforceV2': 'SalesforceV2Source',
      ...
      // SAP Connectors
      'SapTable': 'SapTableSource',
      'SapHana': 'SapHanaSource',
      'SapBw': 'SapBwSource',
      'SapCloudForCustomer': 'SapCloudForCustomerSource',
      'SapEcc': 'SapEccSource',

      // Fabric-Specific
      'LakehouseTable': 'LakehouseTableSource', // OneLake tables
      'DataWarehouse': 'DataWarehouseSource',   // Fabric Warehouse
    };

    return specialMappings[datasetType] || `${datasetType}Source`;
  }
   ...
   * Simplified version from copyActivityTransformer
   */
...
export const lookupActivityTransformer = new LookupActivityTransformer();
