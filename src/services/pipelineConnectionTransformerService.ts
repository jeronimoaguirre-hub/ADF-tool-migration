...
export class PipelineConnectionTransformerService {
  ...
  static transformPipelineWithConnections(
    ...
    const normalizeMappedTarget = (mappedValue: string | undefined): string | undefined => {
      if (!mappedValue) {
        return undefined;
      }

      if (mappedValue.startsWith('lakehouse:')) {
        const lakehouseId = mappedValue.substring('lakehouse:'.length);
        if (lakehouseId) {
          console.log(`  🏠 Lakehouse mapping selected, using lakehouse item id: ${lakehouseId}`);
          return lakehouseId;
        }
      }
    ...
    const findConnectionIdForActivity = (activityName: string, referenceId?: string, linkedServiceName?: string): string | undefined => {
      ...
      if (referenceId && referenceMappings[referenceId]) {
        const resolvedMapping = normalizeMappedTarget(referenceMappings[referenceId]);
        console.log(`  🎯 NEW format hit: ${activityName} (${referenceId}) → ${resolvedMapping}`);
      ...
      if (activityMapping?.selectedConnectionId) {
        const resolvedMapping = normalizeMappedTarget(activityMapping.selectedConnectionId);
        console.log(`  🔄 OLD format hit: ${activityName} → ${resolvedMapping}`);
        return resolvedMapping;
      }
