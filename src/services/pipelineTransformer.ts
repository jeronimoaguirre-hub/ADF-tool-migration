...
import { ExistingConnectionsService } from './existingConnectionsService';
...
export class PipelineTransformer {
  ...
  async createPipeline(
    ...
    try {
      ...
      let response = await fetch(endpoint, { method: 'POST', headers, body: JSON.stringify(pipelinePayload) });

      // Lakehouse fallback: if direct lakehouse item mappings are rejected, resolve and retry using existing Lakehouse connection IDs.
      if (!response.ok) {
        const lakehouseIds = this.extractLakehouseSelections(component.name, connectionMappings);
        if (lakehouseIds.length > 0) {
          const fallbackDefinition = await this.resolveLakehouseMappingsToConnections(
            cleanedDefinition,
            lakehouseIds,
            accessToken,
            workspaceId
          ...
          if (fallbackDefinition) {
            const fallbackBase64Payload = PipelineConnectionTransformerService.generateFabricPipelinePayload(fallbackDefinition);
            const fallbackPayload = {
              ...pipelinePayload,
              definition: { parts: [{ path: 'pipeline-content.json', payload: fallbackBase64Payload, payloadType: 'InlineBase64' }] }
            };

            response = await fetch(endpoint, { method: 'POST', headers, body: JSON.stringify(fallbackPayload) });
          }
        }
      }
  ...
  private extractLakehouseSelections(
    pipelineName: string,
    connectionMappings?: PipelineConnectionMappings
  ): string[] {
    const lakehouseIds = new Set<string>();

    const addIfLakehouse = (value?: string) => {
      if (value && value.startsWith('lakehouse:')) {
        const id = value.substring('lakehouse:'.length);
        if (id) {
          lakehouseIds.add(id);
        }
      }
    ...
    const referenceMappings = this.referenceMappings?.[pipelineName] || {};
    Object.values(referenceMappings).forEach((value) => addIfLakehouse(value));

    const legacyMappings = connectionMappings?.[pipelineName] || {};
    Object.values(legacyMappings).forEach((mapping) => addIfLakehouse(mapping?.selectedConnectionId));

    return Array.from(lakehouseIds);
  }

  private async resolveLakehouseMappingsToConnections(
    pipelineDefinition: any,
    lakehouseIds: string[],
    accessToken: string,
    workspaceId: string
    ...
    try {
      const connections = await ExistingConnectionsService.getExistingConnections(accessToken, workspaceId);
      const replacementMap = new Map<string, string>();

      for (const lakehouseId of lakehouseIds) {
        const matchedConnection = connections.find((connection) =>
          connection.connectionDetails?.type === 'Lakehouse' &&
          typeof connection.connectionDetails?.path === 'string' &&
          connection.connectionDetails.path.includes(lakehouseId)
        );

        if (matchedConnection?.id) {
          replacementMap.set(lakehouseId, matchedConnection.id);
        }
      }
      ...
      const fallbackDefinition = JSON.parse(JSON.stringify(pipelineDefinition));
      const replaceConnectionIds = (activity: any) => {
        if (activity?.externalReferences?.connection && replacementMap.has(activity.externalReferences.connection)) {
          activity.externalReferences.connection = replacementMap.get(activity.externalReferences.connection);
        }

        if (activity?.typeProperties?.datasetSettings?.externalReferences?.connection &&
            replacementMap.has(activity.typeProperties.datasetSettings.externalReferences.connection)) {
          activity.typeProperties.datasetSettings.externalReferences.connection =
            replacementMap.get(activity.typeProperties.datasetSettings.externalReferences.connection);
        }

        if (activity?.typeProperties?.activities) {
          activity.typeProperties.activities.forEach(replaceConnectionIds);
        }
        if (activity?.typeProperties?.ifTrueActivities) {
          activity.typeProperties.ifTrueActivities.forEach(replaceConnectionIds);
        }
        if (activity?.typeProperties?.ifFalseActivities) {
          activity.typeProperties.ifFalseActivities.forEach(replaceConnectionIds);
        }
        if (activity?.typeProperties?.defaultActivities) {
          activity.typeProperties.defaultActivities.forEach(replaceConnectionIds);
        }
        if (activity?.typeProperties?.cases) {
          activity.typeProperties.cases.forEach((caseItem: any) => {
            if (Array.isArray(caseItem.activities)) {
              caseItem.activities.forEach(replaceConnectionIds);
            }
          });
        }
      };

      if (Array.isArray(fallbackDefinition?.properties?.activities)) {
        fallbackDefinition.properties.activities.forEach(replaceConnectionIds);
      }

      return fallbackDefinition;
    } catch (error) {
      console.warn('Failed to resolve lakehouse mappings to existing connections', error);
      return null;
    }
  }
